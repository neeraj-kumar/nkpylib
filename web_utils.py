...
class LLMSearcher(BaseSearcher):
    ...
    kind of query.
    """
    sys_prompt='''You are a helpful search assistant that translates natural language queries into
    structured search queries. You will output a list of structured queries that can be run against
    the full searcher.'''
        ...
        try:
            ...
            for structured_q in lst:
                ...
                try:
                    ...
                    self.add_msg(f'Could not parse {structured_q}: {e}')
                    traceback.print_exc()
            if parsed:
                self.req.searcher_used = self.full_searcher # type: ignore
            else:
                raise ValueError('No valid structured queries generated by LLM')
        except Exception as e:
            self.add_msg(f'Could not run full search, switching to simple search')
...
<html>
  <head>
    ...
    <script src="https://neerajkumar.org/static/js/intersection-observer-admin.js"></script>
